...
if selfie is not None:
    ...
    st.markdown(f"<p style='font-size:20px; color:green;'>Selfie saved successfully.</p>", unsafe_allow_html=True)
...
if uploaded_id1 is not None:
    ...
    st.markdown(f"<p style='font-size:20px; color:green;'>Image uploaded and saved at: {save_path}</p>", unsafe_allow_html=True)
else:
    st.markdown("<p style='font-size:20px; color:blue;'>Please upload or capture an image to continue.</p>", unsafe_allow_html=True)
...
if uploaded_id2 is not None:
    ...
    st.markdown("<p style='font-size:20px; color:blue;'>Please upload or capture an image2 to continue.</p>", unsafe_allow_html=True)

tot_score = 0
button = st.button("Compute Score: ")

if selfie is not None:
    if button and uploaded_id1 is None and uploaded_id2 is None:
        st.markdown("<p style='font-size:20px; color:blue;'>Upload at least one document....</p>", unsafe_allow_html=True)

    elif button and uploaded_id1 is not None and uploaded_id2 is None:
        if len(loc) == 0:
            st.markdown("<p style='font-size:20px; color:blue;'>Face can't be recognized. Upload a clear photo again.</p>", unsafe_allow_html=True)
        else:
            st.markdown("<p style='font-size:20px; color:blue;'>Only one ID is uploaded, computing score.....</p>", unsafe_allow_html=True)
            distance = fr.face_distance([enc], selfie_enc[0])[0]
            st.markdown(f"<p style='font-size:18px;'>Distance: {distance}, Score: {tot_score}</p>", unsafe_allow_html=True)
            if distance < 0.6:
                tot_score = 50

    elif button and uploaded_id1 is None and uploaded_id2 is not None:
        if len(loc2) == 0:
            st.markdown("<p style='font-size:20px; color:blue;'>Face can't be recognized. Upload a clear photo again.</p>", unsafe_allow_html=True)
        else:
            st.markdown("<p style='font-size:20px; color:blue;'>Only one ID is uploaded, computing score.....</p>", unsafe_allow_html=True)
    ...
    elif button and uploaded_id1 is not None and uploaded_id2 is not None:
        scores = {}
        ...
        st.markdown(f"<p style='font-size:18px;'>Scores: {scores}<br>Distance1: {distance}<br>Distance2: {distance2}<br>Total Score: {tot_score}</p>", unsafe_allow_html=True)

    if button:
        if tot_score < 25:
            st.markdown("<p style='font-size:22px; color:red;'><b>KYC Result: FAIL</b></p>", unsafe_allow_html=True)
        else:
            st.markdown("<p style='font-size:22px; color:green;'><b>KYC Result: PASS</b></p>", unsafe_allow_html=True)

else:
    st.markdown("<p style='font-size:20px; color:red;'>Please upload selfie</p>", unsafe_allow_html=True)
